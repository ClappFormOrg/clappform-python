...
- **`location`** is your **tenant**: a client subdomain (e.g. `acme`), sent as
  metadata on *every* call. It selects which tenant database the server uses.
- **`cluster`** is the set of **hosts** that serve that tenant (e.g. `prod`,
...
- **Slug** is a human-readable name for the same entity. `collection(ref)` and
