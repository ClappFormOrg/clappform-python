...
class Clappform:
    ...
    def _write_dataframe_chunk(self, df: DataFrame, colletion: dc.Collection) -> dc.ApiResponse:
    ...
    def write_dataframe(
        ...
        write_chunk = partial(self._write_dataframe_chunk, collection=collection)
        for result in self.executor.map(write_chunk, [df[i : i + size] for i in range(0, df.shape[0], size)]):
            yield result
